...
namespace
{
	...
	void sha512_init(cc_sha512_ctx *ctx)
	{
#ifndef UNROLL_LOOPS
		int i;
		for (i = 0; i < 8; i++) {
	...
	void sha512_update(cc_sha512_ctx *ctx, const unsigned char *message, unsigned int len)
	{
	...
	void sha512_final(cc_sha512_ctx *ctx, unsigned char *digest)
	{
		unsigned int block_nb;
		unsigned int pm_len;
		unsigned int len_b;
