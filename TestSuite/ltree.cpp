...
#include <crisscross/ltree.h>

using namespace CrissCross::Data;
using namespace CrissCross::System;

int TestLTree_CString()
{
	LTree<const char *, const char *> *ltree = new LTree<const char *, const char *>();
	char *strings[TREE_ITEMS], *tmp;

	/* Make sure allocation worked */
	TEST_ASSERT(ltree != NULL);

	/* Make sure the size starts at 0 */
	TEST_ASSERT(ltree->size() == 0);

	memset(strings, 0, sizeof(strings));

	/* Make sure the tree encapsulates keys properly */
	tmp = cc_strdup("testkey");
	ltree->insert(tmp, "encapsulation test");
	free(tmp); tmp = NULL;
	TEST_ASSERT(ltree->size() == 1);
	TEST_ASSERT(ltree->exists("testkey"));
	TEST_ASSERT(ltree->erase("testkey"));
	TEST_ASSERT(!ltree->exists("testkey"));
	TEST_ASSERT(!ltree->erase("testkey"));
	TEST_ASSERT(ltree->size() == 0);
	...
	{
		TEST_ASSERT(ltree->insert(strings[i], strings[(TREE_ITEMS - 1) - i]));
	}
	...
	{
		const char *val = NULL;
		TEST_ASSERT(ltree->exists(strings[i]));
		TEST_ASSERT((val = ltree->find(strings[i])) != NULL);
	...
	for (size_t i = 0; i < TREE_ITEMS; i++)
	{
		TEST_ASSERT(ltree->erase(strings[i]));
		TEST_ASSERT(!ltree->exists(strings[i]));
	}
...
int TestLTree_String()
{
	LTree<std::string, std::string> *ltree = new LTree<std::string, std::string>();
	...
	ltree->insert(tmp, std::string("encapsulation test"));
	tmp = std::string("");
	TEST_ASSERT(ltree->size() == 1);
	TEST_ASSERT(ltree->exists("testkey"));
	TEST_ASSERT(ltree->erase("testkey"));
	...
	for (size_t i = 0; i < TREE_ITEMS; i++)
	{
		...
		TEST_ASSERT(ltree->exists(strings[i]));
		TEST_ASSERT((val = ltree->find(strings[i], "")) != std::string(""));
		TEST_ASSERT(val.length() > 0);
...
int TestLTree_Int()
{
	LTree<int, int> *ltree = new LTree<int, int>();
	...
	/* Make sure the tree encapsulates keys properly */
	tmp = 256;
	ltree->insert(tmp, RandomNumber());
	tmp = 0;
	TEST_ASSERT(ltree->size() == 1);
	TEST_ASSERT(ltree->exists(256));
	TEST_ASSERT(ltree->erase(256));
	TEST_ASSERT(!ltree->exists(256));
	TEST_ASSERT(!ltree->erase(256));
	TEST_ASSERT(ltree->size() == 0);
	...
	for (size_t i = 0; i < TREE_ITEMS; i++)
	{
		TEST_ASSERT(ltree->insert(data[i], data[TREE_ITEMS - 1 - i]));
	}
	...
	{
		int val;
		TEST_ASSERT(ltree->exists(data[i]));
		TEST_ASSERT((val = ltree->find(data[i], -1)) != -1);
	...
	for (size_t i = 0; i < TREE_ITEMS; i++)
	{
		TEST_ASSERT(ltree->erase(data[i]));
		TEST_ASSERT(!ltree->exists(data[i]));
	}
